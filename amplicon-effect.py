...
def get_barcode_fasta(my_fasta):
    ...
    for fasta in fasta_sequences:
        clean_fasta_id = fasta.id.split("_")[0] # to remove _1 added by getorf in description
...
def get_cluster_annot(barcode_cluster_dict, dict_bar_nuc, dict_bar_orf):
        ...
        with tempfile.NamedTemporaryFile(mode = "w") as my_fasta:
            for fasta in fasta_sequences:
                fabarcode = fasta.id
                fabarcode_clean = fabarcode.split("_")[0] # to remove _1 added by getorf in description
                fasta.id = sample_name + "---" + fasta.id
                counter += 1
                SeqIO.write(fasta, my_fasta, "fasta")
                my_fasta.flush()
                output = subprocess.check_output(['diffseq', '-asequence'] + [str(reference_file)] + ['-bsequence'] + [str(my_fasta.name)] + ['-aoutfeat', 'seq1out', '-boutfeat', 'seq2out', '-wordsize', wordsize, '-rdesshow3', '-rformat', 'excel', 'stdout'],stderr = subprocess.STDOUT)
                deleteContent(my_fasta)
                dict_cluster_annot[int(barcode_cluster_dict.get(str(fabarcode_clean)))] = reformat_diffseq_output(output, fabarcode_clean, dict_bar_nuc, dict_bar_orf)
                cluster_number += 1
                bar.update(counter)
        bar.finish()
